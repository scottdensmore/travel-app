...
The instructions for the `ui-review` sub-agent. Its definition at
`.claude/agents/ui-review.md` is a pointer at this file: a definition is injected
...
Read `.claude/agents/ui-review.md`, this file, and `AGENTS.md` from disk when
reasoning about them. What was injected into your context is a snapshot from
when the session first spawned you and is never refreshed, so it can lag the
worktree you are reviewing. If the copy of this role in your context holds
inline instructions rather than a pointer, it predates the split (#246) and
everything in it may be stale; the pointer on disk names where the current
version lives. Read it before trusting anything you were given.
